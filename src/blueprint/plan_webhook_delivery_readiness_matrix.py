"""Build plan-level webhook delivery readiness matrices."""

from __future__ import annotations

from dataclasses import dataclass, field
import re
from typing import Any, Iterable, Literal, Mapping

from blueprint.plan_secrets_rotation_readiness_matrix import _candidate_texts, _dedupe, _evidence_snippet, _markdown_cell, _optional_text, _source_payload, _task_id

WebhookDeliveryReadiness = Literal["ready", "partial", "blocked"]

_READINESS_ORDER: dict[WebhookDeliveryReadiness, int] = {"blocked": 0, "partial": 1, "ready": 2}
_WEBHOOK_RE = re.compile(r"\b(?:webhook|web hook|callback url|event delivery|subscription endpoint|outbound event|inbound event)\b", re.I)
_RETRY_RE = re.compile(r"\b(?:retry|retries|backoff|redeliver|delivery attempt|exponential|at[- ]least[- ]once)\b", re.I)
_SIGN_RE = re.compile(r"\b(?:signature|signing|hmac|secret|verification|verify signature|signed payload)\b", re.I)
_LOG_RE = re.compile(r"\b(?:delivery log|logging|logs|audit|attempt history|metrics|trace|dashboard|observability)\b", re.I)
_REPLAY_RE = re.compile(r"\b(?:replay|redrive|resend|manual delivery|reprocess|requeue)\b", re.I)
_TIMEOUT_RE = re.compile(r"\b(?:timeout|deadline|latency budget|response time|slow consumer|non[- ]2xx|5xx)\b", re.I)
_DLQ_RE = re.compile(r"\b(?:dead[- ]letter|dlq|poison|failed delivery queue|parking lot|quarantine)\b", re.I)
_NOTIFY_RE = re.compile(r"\b(?:notify|notification|email|alert|consumer message|developer portal|status page|changelog)\b", re.I)


@dataclass(frozen=True, slots=True)
class PlanWebhookDeliveryReadinessRow:
    task_id: str
    title: str
    retry_policy: str = "missing"
    signature_verification: str = "missing"
    delivery_logging: str = "missing"
    replay_tooling: str = "missing"
    timeout_handling: str = "missing"
    dead_letter_handling: str = "missing"
    consumer_notification: str = "missing"
    gaps: tuple[str, ...] = field(default_factory=tuple)
    readiness: WebhookDeliveryReadiness = "partial"
    readiness_score: float = 0.0
    evidence: tuple[str, ...] = field(default_factory=tuple)

    def to_dict(self) -> dict[str, Any]:
        return {
            "task_id": self.task_id,
            "title": self.title,
            "retry_policy": self.retry_policy,
            "signature_verification": self.signature_verification,
            "delivery_logging": self.delivery_logging,
            "replay_tooling": self.replay_tooling,
            "timeout_handling": self.timeout_handling,
            "dead_letter_handling": self.dead_letter_handling,
            "consumer_notification": self.consumer_notification,
            "gaps": list(self.gaps),
            "readiness": self.readiness,
            "readiness_score": self.readiness_score,
            "evidence": list(self.evidence),
        }


@dataclass(frozen=True, slots=True)
class PlanWebhookDeliveryReadinessMatrix:
    plan_id: str | None = None
    rows: tuple[PlanWebhookDeliveryReadinessRow, ...] = field(default_factory=tuple)
    webhook_task_ids: tuple[str, ...] = field(default_factory=tuple)
    no_webhook_task_ids: tuple[str, ...] = field(default_factory=tuple)
    summary: dict[str, Any] = field(default_factory=dict)

    @property
    def records(self) -> tuple[PlanWebhookDeliveryReadinessRow, ...]:
        return self.rows

    def to_dict(self) -> dict[str, Any]:
        return {
            "plan_id": self.plan_id,
            "rows": [row.to_dict() for row in self.rows],
            "records": [row.to_dict() for row in self.records],
            "webhook_task_ids": list(self.webhook_task_ids),
            "no_webhook_task_ids": list(self.no_webhook_task_ids),
            "summary": dict(self.summary),
        }

    def to_dicts(self) -> list[dict[str, Any]]:
        return [row.to_dict() for row in self.rows]

    def to_markdown(self) -> str:
        title = "# Plan Webhook Delivery Readiness Matrix"
        if self.plan_id:
            title = f"{title}: {self.plan_id}"
        lines = [title, ""]
        if not self.rows:
            lines.append("No webhook delivery readiness rows were inferred.")
            return "\n".join(lines)
        lines.extend([
            "| Task | Title | Retries | Signing | Logs | Replay | Timeout | DLQ | Notify | Readiness | Score | Gaps |",
            "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        ])
        for row in self.rows:
            lines.append(
                "| "
                f"`{_markdown_cell(row.task_id)}` | {_markdown_cell(row.title)} | {row.retry_policy} | "
                f"{row.signature_verification} | {row.delivery_logging} | {row.replay_tooling} | "
                f"{row.timeout_handling} | {row.dead_letter_handling} | {row.consumer_notification} | "
                f"{row.readiness} | {row.readiness_score:.2f} | {_markdown_cell('; '.join(row.gaps) or 'none')} |"
            )
        return "\n".join(lines)


def build_plan_webhook_delivery_readiness_matrix(source: Any) -> PlanWebhookDeliveryReadinessMatrix:
    plan_id, tasks = _source_payload(source)
    rows: list[PlanWebhookDeliveryReadinessRow] = []
    no_ids: list[str] = []
    for index, task in enumerate(tasks, start=1):
        row = _task_row(task, index)
        if row:
            rows.append(row)
        else:
            no_ids.append(_task_id(task, index))
    rows.sort(key=lambda row: (_READINESS_ORDER[row.readiness], row.task_id))
    result = tuple(rows)
    return PlanWebhookDeliveryReadinessMatrix(plan_id, result, tuple(row.task_id for row in result), tuple(no_ids), _summary(len(tasks), result))


def analyze_plan_webhook_delivery_readiness_matrix(source: Any) -> PlanWebhookDeliveryReadinessMatrix:
    if isinstance(source, PlanWebhookDeliveryReadinessMatrix):
        return source
    return build_plan_webhook_delivery_readiness_matrix(source)


def generate_plan_webhook_delivery_readiness_matrix(source: Any) -> PlanWebhookDeliveryReadinessMatrix:
    return build_plan_webhook_delivery_readiness_matrix(source)


def plan_webhook_delivery_readiness_matrix_to_dict(matrix: PlanWebhookDeliveryReadinessMatrix) -> dict[str, Any]:
    return matrix.to_dict()


plan_webhook_delivery_readiness_matrix_to_dict.__test__ = False


def plan_webhook_delivery_readiness_matrix_to_dicts(matrix: PlanWebhookDeliveryReadinessMatrix | Iterable[PlanWebhookDeliveryReadinessRow]) -> list[dict[str, Any]]:
    if isinstance(matrix, PlanWebhookDeliveryReadinessMatrix):
        return matrix.to_dicts()
    return [row.to_dict() for row in matrix]


plan_webhook_delivery_readiness_matrix_to_dicts.__test__ = False


def plan_webhook_delivery_readiness_matrix_to_markdown(matrix: PlanWebhookDeliveryReadinessMatrix) -> str:
    return matrix.to_markdown()


plan_webhook_delivery_readiness_matrix_to_markdown.__test__ = False


def _task_row(task: Mapping[str, Any], index: int) -> PlanWebhookDeliveryReadinessRow | None:
    texts = _candidate_texts(task)
    if not any(_WEBHOOK_RE.search(text) for _, text in texts):
        return None
    statuses = {
        "retry_policy": _status(_RETRY_RE, texts),
        "signature_verification": _status(_SIGN_RE, texts),
        "delivery_logging": _status(_LOG_RE, texts),
        "replay_tooling": _status(_REPLAY_RE, texts),
        "timeout_handling": _status(_TIMEOUT_RE, texts),
        "dead_letter_handling": _status(_DLQ_RE, texts),
        "consumer_notification": _status(_NOTIFY_RE, texts),
    }
    gaps = tuple(f"Missing {label}." for field, label in (
        ("retry_policy", "retry policy"),
        ("signature_verification", "signature verification"),
        ("delivery_logging", "delivery logging"),
        ("replay_tooling", "replay tooling"),
        ("timeout_handling", "timeout handling"),
        ("dead_letter_handling", "dead-letter handling"),
        ("consumer_notification", "consumer notification"),
    ) if statuses[field] == "missing")
    score = round(sum(1 for status in statuses.values() if status == "present") / len(statuses), 2)
    if statuses["retry_policy"] == "missing" or statuses["signature_verification"] == "missing":
        readiness: WebhookDeliveryReadiness = "blocked"
    elif gaps:
        readiness = "partial"
    else:
        readiness = "ready"
    return PlanWebhookDeliveryReadinessRow(
        task_id=_task_id(task, index),
        title=_optional_text(task.get("title")) or _task_id(task, index),
        gaps=gaps,
        readiness=readiness,
        readiness_score=score,
        evidence=tuple(_dedupe(_evidence_snippet(field, text) for field, text in texts if _WEBHOOK_RE.search(text))),
        **statuses,
    )


def _status(pattern: re.Pattern[str], texts: Iterable[tuple[str, str]]) -> str:
    return "present" if any(pattern.search(text) for _, text in texts) else "missing"


def _summary(task_count: int, rows: Iterable[PlanWebhookDeliveryReadinessRow]) -> dict[str, Any]:
    row_list = list(rows)
    return {
        "task_count": task_count,
        "row_count": len(row_list),
        "webhook_task_count": len(row_list),
        "no_webhook_task_count": task_count - len(row_list),
        "readiness_counts": {name: sum(1 for row in row_list if row.readiness == name) for name in _READINESS_ORDER},
        "average_readiness_score": round(sum(row.readiness_score for row in row_list) / len(row_list), 2) if row_list else 0.0,
    }


__all__ = [
    "PlanWebhookDeliveryReadinessMatrix",
    "PlanWebhookDeliveryReadinessRow",
    "WebhookDeliveryReadiness",
    "analyze_plan_webhook_delivery_readiness_matrix",
    "build_plan_webhook_delivery_readiness_matrix",
    "generate_plan_webhook_delivery_readiness_matrix",
    "plan_webhook_delivery_readiness_matrix_to_dict",
    "plan_webhook_delivery_readiness_matrix_to_dicts",
    "plan_webhook_delivery_readiness_matrix_to_markdown",
]
