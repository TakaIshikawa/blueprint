...
def test_plan_graph_outputs_dot_with_milestone_clusters(tmp_path, monkeypatch):
    _write_config(tmp_path, monkeypatch)
    store = init_db(str(tmp_path / "blueprint.db"))
    store.insert_implementation_brief(_implementation_brief())
    store.insert_execution_plan(_execution_plan(), _tasks())

    result = CliRunner().invoke(cli, ["plan", "graph", "plan-test"])

    assert result.exit_code == 0, result.output
    assert result.output.startswith('digraph "plan-test" {\n')
    assert "subgraph cluster_Foundation" in result.output
    assert "subgraph cluster_Interface" in result.output
    assert '"task-setup" [label="task-setup\\nSetup project\\nStatus: completed"' in result.output
    assert '"task-api" [label="task-api\\nBuild API\\nStatus: in_progress"' in result.output
    assert '"task-setup" -> "task-api";' in result.output
    assert '"task-api" -> "task-ui";' in result.output


def test_plan_graph_writes_json_output_file(tmp_path, monkeypatch):
    _write_config(tmp_path, monkeypatch)
    output_path = tmp_path / "graphs" / "plan-graph.json"
    store = init_db(str(tmp_path / "blueprint.db"))
    store.insert_implementation_brief(_implementation_brief())
    store.insert_execution_plan(_execution_plan(), _tasks())
    ...
    assert result.exit_code == 0, result.output
    assert "Exported JSON graph to:" in result.output

    payload = json.loads(output_path.read_text())
    assert payload["plan_id"] == "plan-test"
    assert [milestone["name"] for milestone in payload["milestones"]] == [
        "Foundation",
        "Interface",
    ]
...
def test_plan_graph_rejects_unknown_dependency_ids(tmp_path, monkeypatch):
    _write_config(tmp_path, monkeypatch)
    store = init_db(str(tmp_path / "blueprint.db"))
    store.insert_implementation_brief(_implementation_brief())
    tasks = _tasks()
    ...
    result = CliRunner().invoke(cli, ["plan", "graph", "plan-test", "--format", "json"])
