...
def test_teamwork_csv_exporter_writes_stable_headers_and_task_rows(tmp_path):
    output_path = tmp_path / "teamwork.csv"

    TeamworkCsvExporter().export(_execution_plan(), _implementation_brief(), str(output_path))

    rows = _read_csv(output_path)
    assert _read_headers(output_path) == TeamworkCsvExporter.FIELDNAMES
    assert len(rows) == 5
...
def test_teamwork_csv_exporter_preserves_dependencies_and_milestone_grouping(tmp_path):
    output_path = tmp_path / "teamwork.csv"

    TeamworkCsvExporter().export(_execution_plan(), _implementation_brief(), str(output_path))

    rows = _read_csv(output_path)
    assert [row["Task List"] for row in rows] == [
        "Foundation",
        "Delivery",
        "Delivery",
        "Delivery",
...
def test_teamwork_csv_exporter_maps_metadata_status_tags_and_context(tmp_path):
    output_path = tmp_path / "teamwork.csv"

    TeamworkCsvExporter().export(_execution_plan(), _implementation_brief(), str(output_path))

    rows = _read_csv(output_path)
    assert [row["Progress"] for row in rows] == ["0", "50", "0", "100", "0"]
    assert [row["Priority"] for row in rows] == ["High", "High", "High", "Low", "Medium"]
...
def test_teamwork_csv_validation_passes_for_rendered_export():
    result = validate_export(_execution_plan(), _implementation_brief(), "teamwork-csv")

    assert result.passed
    assert result.findings == []


def test_teamwork_csv_validation_reports_missing_required_headers(tmp_path):
    output_path = tmp_path / "teamwork.csv"
    output_path.write_text(
        "Task Name,Description,Task List,Priority,Progress,Tags,Predecessors,External ID\n"
        "Setup project,Description,Foundation,High,0,setup,,task-setup\n"
    )
    ...
    assert findings[0].code == "teamwork_csv.missing_column"
    assert "Estimated Time" in findings[0].message
    assert any(finding.code == "teamwork_csv.row_count_mismatch" for finding in findings)


def test_teamwork_csv_validation_reports_row_count_missing_and_duplicate_tasks(tmp_path):
    output_path = tmp_path / "teamwork.csv"
    with output_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=TeamworkCsvExporter.FIELDNAMES)
        writer.writeheader()
    ...
    assert [finding.code for finding in findings] == [
        "teamwork_csv.row_count_mismatch",
        "teamwork_csv.missing_tasks",
        "teamwork_csv.duplicate_task_id",
    ]
    assert "task-api" in findings[1].message


def test_export_run_teamwork_csv_writes_file_and_records_csv_export(tmp_path, monkeypatch):
    store = _setup_store(tmp_path, monkeypatch)
    store.insert_implementation_brief(_implementation_brief())
    plan_id = store.insert_execution_plan(_execution_plan(include_tasks=False), _tasks())

    result = CliRunner().invoke(cli, ["export", "run", plan_id, "--target", "teamwork-csv"])

    assert result.exit_code == 0, result.output
    output_path = tmp_path / "exports" / f"{plan_id}-teamwork-csv.csv"
    ...
    assert records[0]["target_engine"] == "teamwork-csv"
...
def _tasks():
    return [
        ...
        },
        {
            ...
            "suggested_engine": "codex",
            "depends_on": ["task-api"],
            "files_or_modules": ["docs/teamwork.md"],
...
def _implementation_brief():
    return {
        ...
        "problem_statement": "Need a Teamwork CSV export",
        "mvp_goal": "Export tasks for Teamwork import",
        "product_surface": "CLI",
        "scope": ["Teamwork CSV exporter"],
        "non_goals": ["Teamwork API automation"],
